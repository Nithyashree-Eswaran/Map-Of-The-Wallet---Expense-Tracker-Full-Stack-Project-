...
public class JwtUtils {
    ...
    try {
        logger.info("Validating JWT Token: {}", authToken); // ADD LOGGING
        ...
        if (authToken.chars().filter(ch -> ch == '.').count() != 2) {
            logger.error("Invalid JWT token format: JWT strings must contain exactly 2 period characters. Found: {}", authToken);
            return false;
        }
